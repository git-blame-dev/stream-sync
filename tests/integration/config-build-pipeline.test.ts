...
const { buildConfig: _buildConfig } = require('../../src/core/config-builders.ts');
...
describe('Config build pipeline integration', () => {
    describe('buildConfig includes all sections required by main.js', () => {
        it('contains all config sections accessed by main.js', () => {
            ...
            expect(typeof built.logging.console.level).toBe('string');
            expect(typeof built.general.viewerCountPollingIntervalMs).toBe('number');
        });
