...
type UpdateGlobalCommandCooldown = (
  commandName: string | null | undefined,
) => void;
...
describe("Command Cooldown Integration", () => {
  ...
  beforeEach(() => {
    ...
    updateGlobalCommandCooldown =
      commandParserModule.updateGlobalCommandCooldown;
    clearExpiredGlobalCooldowns =
      commandParserModule.clearExpiredGlobalCooldowns;
  });
  ...
  describe("when handling rapid command succession", () => {
    testWithTimeout(
      "should handle multiple rapid cooldown checks correctly",
      () => {
    ...
    testWithTimeout(
      "should maintain separate cooldowns for different commands",
      () => {
        ...
        const results = commands.map((cmd) =>
          checkGlobalCommandCooldown(cmd, 2000),
        );
  ...
  describe("when memory management is required", () => {
    ...
    testWithTimeout(
      "should handle cleanup without affecting active cooldowns",
      () => {
  ...
  describe("when handling edge cases", () => {
    testWithTimeout(
      "should handle concurrent cooldown operations safely",
      () => {
    ...
    testWithTimeout("should handle malformed command names gracefully", () => {
      const malformedCommands: Array<string | null | undefined> = [
        null,
        undefined,
        "",
        "   ",
    ...
    testWithTimeout("should handle extreme cooldown values", () => {
      ...
      const longCooldown = checkGlobalCommandCooldown(
        "!extreme",
  ...
  describe("when integrating with configuration system", () => {
    testWithTimeout(
      "should respect different cooldown periods per platform",
      () => {
        ...
        platforms.forEach((platform, index) => {
          ...
          const isBlocked = checkGlobalCommandCooldown(
            commandName,
            cooldownPeriods[index],
          );
          expect(isBlocked).toBe(true);
        });
      },
    ...
    testWithTimeout(
      "should handle config changes without losing existing cooldowns",
      () => {
