...
} from "../../src/utils/tiktok-data-extraction";
...
describe("TikTok End-to-End Unknown User Fix Integration", () => {
  ...
  describe("Complete TikTok Gift Processing Pipeline", () => {
    describe("when processing actual TikTok gift event", () => {
      test(
        "should show actual gift sender username in final notification",
        () => {
          ...
          expect(giftNotification.displayMessage).toBe(
            "TestGiftSender sent 5x TestGiftAlpha",
          );
      ...
      test(
        "should handle combo gifts with actual usernames",
        () => {
          const comboGiftData = {
            user: {
          ...
          expect(comboNotification.displayMessage).toBe(
            "TestComboSender finished combo: 1x TestGiftCombo",
          );
          expectNoTechnicalArtifacts(comboNotification.displayMessage);
          expect(comboNotification.displayMessage).not.toContain(
            "Unknown User",
          );
        },
        { timeout: TEST_TIMEOUTS.INTEGRATION },
  ...
  describe("Error Recovery and Fallback Behavior", () => {
    describe("when TikTok data is partially corrupted", () => {
      test(
        "should gracefully handle missing username fields",
        () => {
          const corruptedData = {
            ...
            user: {
              ...
              profilePictureUrl:
                "https://example.invalid/avatar-missing-username.jpg",
            },
