...
import { createMockOBSConnection, noOpLogger } from "../helpers/mock-factories";
...
describe("Monetisation pipeline integration", () => {
    ...
    displayQueue.processQueue = createMockFn(async () => {
      while (displayQueue.queue.length > 0) {
        ...
        const commandKey =
          ...
          (item.type === "platform:paypiggy" ? "paypiggies" : "gifts");
  ...
  test.each(flows)(
    ...
    async ({
      ...
      for (
        let attempt = 0;
        attempt < 10 && displayQueue.addItem.mock.calls.length === 0;
        attempt += 1
      ) {
        await new Promise(setImmediate);
      }
