...
import { clearAllMocks, restoreAllMocks } from "../helpers/bun-mock-utils";
...
describe("Extractor Service Integration", () => {
  ...
  describe("Validation and Content Quality", () => {
    test("validates extracted viewer counts", () => {
      ...
      testCases.forEach((testCase) => {
        const isValid = YouTubeViewerExtractor.isValidViewerCount(
          testCase.count,
        );
    ...
    test("handles international number formats", () => {
      ...
      const problematicFormats = ["1.234 watching now", "1 234 watching now"];
  ...
  describe("Performance and Efficiency", () => {
    ...
    test("handles high-frequency extraction calls", () => {
      ...
      const startTime = testClock.now();
      const results: Array<
        ReturnType<typeof YouTubeViewerExtractor.extractConcurrentViewers>
      > = [];
