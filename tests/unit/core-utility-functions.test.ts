...
describe("Core Utility Functions", () => {
  describe("Adaptive Retry System", () => {
    ...
    test(
      "ADAPTIVE_RETRY_CONFIG should be exported with correct configuration",
      () => {
    ...
    test(
      "calculateAdaptiveRetryDelay should calculate initial delay correctly",
      () => {
    ...
    test(
      "incrementRetryCount should increase the retry count and return a new delay",
      () => {
    ...
    test(
      "resetRetryCount should reset the count for a specific platform",
      () => {
    ...
    test(
      "calculateAdaptiveRetryDelay should respect the MAX_DELAY",
      () => {
        ...
        const finalCalculatedDelay =
          retrySystem.calculateAdaptiveRetryDelay(platform);
    ...
    test(
      "getRetryCount should return 0 for a platform that has not retried",
      () => {
    ...
    test(
      "Retry counts should be independent across platforms",
      () => {
