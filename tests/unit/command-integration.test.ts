...
describe("Command Integration System", () => {
  ...
  beforeEach(() => {
    configFixture = {
      commands: {
        ...
        hackerman2:
          "!hacker|!hacks, vfx center green, hacks|hacker|cheats|cheater|cheating|aimbot|walls",
  ...
  describe("VFX Configuration", () => {
    ...
    test("should get VFX config for keyword-based commands", () => {
      const config = commandParser.getVFXConfig(
        "i",
    ...
    test("should handle case-insensitive matching", () => {
      ...
      const config2 = commandParser.getVFXConfig(
        "!hello",
        "I am a MOD and I approve this message",
      );
  ...
  describe("Farewell Command Detection", () => {
    test("should detect farewell commands by trigger", () => {
      const farewellMatch = commandParser.getMatchingFarewell(
        "!bye everyone!",
        "!bye",
      );
    ...
    test("should detect farewell commands by keyword", () => {
      ...
      const farewellMatch = farewellParser.getMatchingFarewell(
    ...
    test("skips invalid command payloads and surfaces execution failures", async () => {
      ...
      await expect(
        runCommand(
          ...
          "/fallback/vfx",
          failingEffectsManager,
        ),
      ).rejects.toThrow("test-run-command-failure");
    });
  });
});
