...
import { createTwitchEventSubSubscriptions } from "../../../../../src/platforms/twitch/connections/eventsub-subscriptions.ts";
...
describe("Twitch EventSub subscriptions", () => {
  test("builds subscription definitions with correct conditions and handlers", () => {
    ...
    expect(byType("channel.subscription.gift").handler).toBe(
      "handlePaypiggyGift",
    );
    expect(byType("channel.subscription.message").handler).toBe(
      "handlePaypiggyMessage",
    );
    ...
    expect(
      byType("channel.follow").getCondition({
        userId: testUserId,
        broadcasterId: testBroadcasterId,
      }),
    ).toEqual({
    ...
    expect(
      byType("channel.raid").getCondition({ broadcasterId: testBroadcasterId }),
    ).toEqual({
      to_broadcaster_user_id: testBroadcasterId,
    });
    expect(
      byType("stream.online").getCondition({
        broadcasterId: testBroadcasterId,
      }),
    ).toEqual({
      broadcaster_user_id: testBroadcasterId,
