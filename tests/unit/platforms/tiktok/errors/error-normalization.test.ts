...
describe("TikTokPlatform error normalization", () => {
  ...
  describe("_normalizeErrorDetails", () => {
    it('returns "Unknown error" for null/undefined input', () => {
      ...
      expect(platform._normalizeErrorDetails(null).message).toBe(
        "Unknown error",
      );
      expect(platform._normalizeErrorDetails(undefined).message).toBe(
        "Unknown error",
      );
    });
  ...
  describe("_normalizeConnectionIssue", () => {
    it('returns "Unknown disconnect reason" for null/undefined', () => {
      ...
      expect(platform._normalizeConnectionIssue(null).message).toBe(
        "Unknown disconnect reason",
      );
      expect(platform._normalizeConnectionIssue(undefined).message).toBe(
        "Unknown disconnect reason",
      );
    });
  ...
  describe("_isStreamNotLive", () => {
    ...
    it('returns true when message contains "not live"', () => {
      ...
      expect(platform._isStreamNotLive({ message: "Stream is not live" })).toBe(
        true,
      );
      expect(platform._isStreamNotLive("User is NOT LIVE right now")).toBe(
        true,
      );
    });
  ...
  describe("_formatStreamNotLiveMessage", () => {
    it("includes code suffix when present", () => {
      ...
      expect(result).toBe(
        "Stream is not live for TikTok user 'testStreamer' (code 4404)",
      );
    });
  ...
  describe("_isRecoverableError", () => {
    ...
    it('returns false for "invalid username"', () => {
      ...
      expect(platform._isRecoverableError("Invalid username provided")).toBe(
        false,
      );
    });
    ...
    it('returns false for "user not found"', () => {
      ...
      expect(platform._isRecoverableError("User not found on TikTok")).toBe(
        false,
      );
    });
    ...
    it('returns false for "private account"', () => {
      ...
      expect(platform._isRecoverableError("This is a private account")).toBe(
        false,
      );
    });
