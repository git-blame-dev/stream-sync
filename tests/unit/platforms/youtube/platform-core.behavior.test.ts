...
describe("YouTubePlatform behavior", () => {
  ...
  it("throws when dependencies argument is not an object", () => {
    expect(() => new YouTubePlatform({}, "bad")).toThrow(
  ...
  it("fails fast when stream detection throws", async () => {
    const streamDetectionService = createStreamDetectionService({
      detectLiveStreams: createMockFn().mockRejectedValue(
  ...
  it("skips remove/delete chat actions in message filtering", () => {
    ...
    const results = skipTypes.map((type) =>
      platform._shouldSkipEvent({ type }),
    );
    ...
    await platform.handleSuperChat({
      item: {
        ...
        author: {
          ...
          name: "AvatarViewer",
          thumbnails: [
            { url: "https://example.invalid/youtube-monetization-avatar.jpg" },
          ],
        },
    ...
    await platform.handleSuperSticker({
      item: {
        ...
        author: {
          ...
          thumbnails: [
            {
              url: "https://example.invalid/youtube-monetization-sticker-avatar.jpg",
            },
          ],
        },
        sticker: {
  ...
  it("requests one immediate refresh for duplicate terminal disconnect events", async () => {
    ...
    const first = await platform.disconnectFromYouTubeStream(
      "video-1",
      "stream ended",
      {
    ...
    const second = await platform.disconnectFromYouTubeStream(
      "video-1",
      "stream ended",
      {
        requestImmediateRefresh: true,
    ...
    expect(refreshCalls).toEqual([
      { videoId: "video-1", reason: "stream ended", source: "livechat-end" },
    ]);
  });
  ...
  it("does not block disconnect completion when refresh is requested during active check", async () => {
    ...
    const disconnectPromise = platform.disconnectFromYouTubeStream(
      "video-3",
      "stream ended",
      {
        requestImmediateRefresh: true,
