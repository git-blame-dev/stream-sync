...
describe('ViewerCountProvider error handler integration', () => {
    ...
    it('logs provider errors at error level and returns 0', async () => {
        ...
        const apiClient = {
            getStreamInfo: createMockFn().mockRejectedValue(new Error('test network timeout'))
        };
        const provider = new TwitchViewerCountProvider(apiClient, {}, { channel: 'test-channel' }, null, mockLogger);
    ...
    it('logs errors at error level across consecutive failures', async () => {
        ...
        const provider = new TikTokViewerCountProvider(platform, { logger: mockLogger });
