...
describe("NotificationBuilder", () => {
  ...
  test("builds Twitch bits inline parts when giftImageUrl is available", () => {
    const notification = NotificationBuilder.build({
      ...
      message: "keep going",
      giftImageUrl:
        "https://example.invalid/twitch/cheer-234-dark-animated-3.gif",
      cheermoteInfo: {
        cleanPrefix: "Cheer",
        tier: 100,
        isMixed: false,
    ...
    expect(notification.displayMessage).toBe(
      "test-twitch-user sent 234 bits: keep going",
    );
  });
  ...
  test("builds YouTube Super Sticker inline parts with image first", () => {
    ...
    expect(notification.parts).toEqual([
      ...
      {
        type: "text",
        text: " Pear character lifting some weights saying 'Keep it up'",
      },
    ]);
  });
