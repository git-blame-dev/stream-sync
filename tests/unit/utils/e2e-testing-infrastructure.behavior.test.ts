...
} from "../../helpers/e2e-testing-infrastructure";
...
describe("e2e-testing-infrastructure behavior", () => {
  ...
  describe("WebSocketMessageSimulator", () => {
    it("processes messages through platform handler and emits success event", async () => {
      ...
      const result = await simulator.injectRawWebSocketMessage(
        { data: 1 },
        platform,
      );
    ...
    it("routes errors through platform error handler and emits error event", async () => {
      ...
      await expect(
        simulator.injectRawWebSocketMessage({ bad: true }, platform),
  ...
  describe("CrossPlatformIntegrationTester", () => {
    it("processes simultaneous events across platforms and returns per-platform results", async () => {
      const tester = new CrossPlatformIntegrationTester(
        { twitch: fakePlatform("t-ok") },
        { logger: noOpLogger },
      );

      const outcome = await tester.processSimultaneousEvents({
        twitch: { foo: "bar" },
      });
    ...
    it("throws when platform is missing from registry", async () => {
      ...
      await expect(
        tester.processSimultaneousEvents({ twitch: { foo: "bar" } }),
      ).rejects.toThrow("Platform twitch not available");
    });
  });
  ...
  describe("UserJourneyValidator", () => {
    it("validates journey success through all stages", async () => {
      ...
      const journey = await validator.validateCompleteUserJourney(
        {
          platform: "twitch",
          rawWebSocketData: { subscription_type: "chat", event: {} },
        },
    ...
    it("flags content quality failures for missing content and malicious links", async () => {
      const validator = new UserJourneyValidator({ logger: noOpLogger });
      const result = await validator.validateContentQualityInFlow({
        message: "visit https://malicious-site.example.invalid",
      });

      expect(result.passed).toBe(false);
      expect(result.blockedElements).toContain(
        "https://malicious-site.example.invalid",
      );
      expect(
        result.checks.some(
          (c) => c.name === "malicious_link_detection" && !c.passed,
        ),
      ).toBe(true);
    });
  });
