...
import { normalizeGreetingIdentityKey } from "../../../src/utils/greeting-identity-key-normalizer";
describe("normalizeGreetingIdentityKey", () => {
  it("normalizes casing and whitespace for greeting identity keys", () => {
    expect(normalizeGreetingIdentityKey("twitch", "  TeSt-User  ")).toBe(
      "test-user",
    );
    expect(normalizeGreetingIdentityKey("tiktok", "  TestUniqueId  ")).toBe(
      "testuniqueid",
    );
  });

  it("strips leading @ only for youtube greeting identity keys", () => {
    expect(normalizeGreetingIdentityKey("youtube", "  @@@TestUser  ")).toBe(
      "testuser",
    );
    expect(normalizeGreetingIdentityKey("twitch", "@@TestUser")).toBe(
      "@@testuser",
    );
    expect(normalizeGreetingIdentityKey("tiktok", "@@TestUser")).toBe(
      "@@testuser",
    );
  });
