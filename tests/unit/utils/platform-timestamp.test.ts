...
} from "../../../src/utils/platform-timestamp";
describe("TikTok timestamp resolution", () => {
  describe("resolveTikTokTimestampMs", () => {
    ...
    it("returns milliseconds unchanged from common.clientSendTime", () => {
      ...
      expect(
        resolveTikTokTimestampMs({ common: { clientSendTime: millis } }),
      ).toBe(millis);
    });
    ...
    it("converts microsecond timestamps down to milliseconds", () => {
      ...
      expect(resolveTikTokTimestampMs({ common: { createTime: micro } })).toBe(
        1_700_000_000_000,
      );
    });
    ...
    it("parses numeric string timestamps", () => {
      expect(resolveTikTokTimestampMs({ timestamp: "1700000000000" })).toBe(
        1_700_000_000_000,
      );
    });
    ...
    it("parses ISO timestamps from common.createTime", () => {
      const iso = "2026-01-04T09:43:46.004Z";
      expect(resolveTikTokTimestampMs({ common: { createTime: iso } })).toBe(
        Date.parse(iso),
      );
    });
    ...
    it("returns null for missing or invalid values", () => {
      ...
      expect(
        resolveTikTokTimestampMs({ common: { createTime: "abc" } }),
      ).toBeNull();
      expect(
        resolveTikTokTimestampMs({ common: { createTime: 0 } }),
      ).toBeNull();
    });
  });
  ...
  describe("resolveTikTokTimestampISO", () => {
    it("returns ISO string when timestamp present", () => {
      const iso = resolveTikTokTimestampISO({
  ...
  describe("resolveYouTubeTimestampISO", () => {
    ...
    it("handles string timestamp_usec", () => {
      ...
      const result = resolveYouTubeTimestampISO({
        timestamp_usec: micros.toString(),
      });
      expect(result).toBe(new Date(Math.floor(micros / 1000)).toISOString());
    });
    ...
    it("returns null for invalid data", () => {
      ...
      expect(
        resolveYouTubeTimestampISO({ timestamp_usec: "invalid" }),
      ).toBeNull();
