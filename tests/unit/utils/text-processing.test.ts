...
describe("Text Processing", () => {
  ...
  describe("when extracting message text", () => {
    ...
    it("should extract text from YouTube message array", () => {
      const messageParts = [{ text: "Hello" }, { text: " world!" }];

      const extracted = textProcessing.extractMessageText(
        messageParts,
        "youtube",
      );
    ...
    it("should handle mixed message part types", () => {
      const messageParts = ["Hello", { text: " world" }, "!"];
  ...
  describe("sanitizeUsername hardening", () => {
    it("removes HTML/JS while preserving readable text and emoji", () => {
      ...
      const result = manager.sanitizeUsername(
        "<b>Alice</b><script>javascript:alert(1)</script> 😃",
      );
  ...
  describe("determineGiftName behavior", () => {
    ...
    it("falls back to generic gift or unknown when no mapping exists", () => {
      expect(manager.determineGiftName({}, "tiktok", "platform:gift")).toBe(
        "gift",
      );
      expect(manager.determineGiftName({}, "twitch", "platform:paypiggy")).toBe(
        "unknown",
      );
    });
  });
  ...
  describe("truncate/slug edge cases", () => {
    ...
    it("generates slug with mixed emoji/HTML/JS removed", () => {
      const slug = textProcessing.toSlug(
        "Hello <b>World</b> 😀 javascript:alert(1)",
      );
    ...
    it("extracts hashtags while ignoring URLs and HTML/script fragments", () => {
      const hashtags = textProcessing.extractHashtags(
        "#fun https://example.com <b>#tag</b> javascript:alert(1) #safe",
      );
    ...
    it("creates slugs that drop URLs and emoji while collapsing punctuation", () => {
      const slug = textProcessing.toSlug(
        "Hello!!! https://example.com 😀 script:alert(1)",
      );
    ...
    it("extracts hashtags and ignores emoji plus javascript urls", () => {
      const hashtags = textProcessing.extractHashtags(
        "#wow 🚀 #cool javascript:alert(1) #end",
      );
    ...
    it("returns ellipsis when preserveWords cannot fit any word", () => {
      const truncated = textProcessing.truncateText(
    ...
    it("truncates long text without preserving words and preserves emoji safely", () => {
      const truncated = textProcessing.truncateText(
        "😀Emoji start with verylongwordthatexceedslimit",
        10,
        false,
      );
    ...
    it("ignores malformed hashtag fragments and scripts", () => {
      expect(textProcessing.extractHashtags("# <script>#bad</script>")).toEqual(
        ["#bad"],
      );
    });
  ...
  describe("text cleaning and extraction edge cases", () => {
    it("normalizes whitespace and strips HTML/JS safely", () => {
      const cleaned = textProcessing.cleanText(
    ...
    it("extracts hashtags while ignoring emoji and javascript fragments", () => {
      const hashtags = textProcessing.extractHashtags(
        "#fun 🚀 #rocket-launch <b>#html</b> javascript:alert(1) #ok",
      );
    ...
    it("creates slugs that drop emoji and strip script markup", () => {
      const slug = textProcessing.toSlug(
        "😀 Fun Time <script>alert(1)</script> https://example.com/path",
      );
  ...
  describe("when formatting chat messages", () => {
    ...
    it("should handle different message types", () => {
      const giftFormatted = textProcessing.formatChatMessage(
        "gift",
        "gifter",
        "Rose",
      );
      const followFormatted = textProcessing.formatChatMessage(
        "follow",
        "follower",
        "",
      );
    ...
    it("should handle short usernames correctly regardless of truncateUsername setting", () => {
      ...
      const formattedTTS = textProcessing.formatChatMessage(
        "chat",
        shortUsername,
        "Hello!",
        {},
      ...
      const formattedDisplay = textProcessing.formatChatMessage(
        "chat",
        shortUsername,
        "Hello!",
        {},
    ...
    it("should work with different message types and truncation settings", () => {
      ...
      const giftTTS = textProcessing.formatChatMessage(
        "gift",
        longUsername,
        "Rose",
        {},
      ...
      const giftDisplay = textProcessing.formatChatMessage(
        "gift",
        longUsername,
        "Rose",
        {},
      ...
      const notificationTTS = textProcessing.formatChatMessage(
        "notification",
        longUsername,
        "alert",
        {},
      ...
      const notificationDisplay = textProcessing.formatChatMessage(
        "notification",
        longUsername,
        "alert",
        {},
  ...
  describe("when converting to title case", () => {
    it("should convert text to title case", () => {
      ...
      expect(textProcessing.toTitleCase("THE QUICK BROWN FOX")).toBe(
        "The Quick Brown Fox",
      );
    });
  ...
  describe("when cleaning text", () => {
    it("should remove HTML tags", () => {
      expect(textProcessing.cleanText("<b>Hello</b> world")).toBe(
        "Hello world",
      );
      expect(textProcessing.cleanText('<script>alert("xss")</script>')).toBe(
        'alert("xss")',
      );
    });
    ...
    it("should handle special characters", () => {
      expect(textProcessing.cleanText("Hello &amp; world")).toBe(
        "Hello & world",
      );
      expect(textProcessing.cleanText("Hello&#39;s world")).toBe(
        "Hello's world",
      );
    });
  ...
  describe("when extracting hashtags", () => {
    ...
    it("should ignore hashtags inside URLs and HTML", () => {
      const hashtags = textProcessing.extractHashtags(
        "Visit https://example.com/#fragment and <b>#html</b>",
      );
    ...
    it("should ignore hashtags that are part of javascript urls", () => {
      const hashtags = textProcessing.extractHashtags(
        "Click javascript:alert(1)#bad then #good",
      );
    ...
    it("should keep emoji-adjacent hashtags after stripping html and script-like urls", () => {
      const hashtags = textProcessing.extractHashtags(
        "🔥#fire🔥 <span>#hidden</span> javascript:alert(1)#bad #ok",
      );
  ...
  describe("when extracting mentions", () => {
    ...
    it("should ignore emails and punctuation around mentions", () => {
      const mentions = textProcessing.extractMentions(
        "Contact @user, email test@example.com and @admin.",
      );
  ...
  describe("when converting to slug", () => {
    it("should convert text to URL-friendly slug", () => {
      ...
      expect(textProcessing.toSlug("The Quick Brown Fox")).toBe(
        "the-quick-brown-fox",
      );
    });
    ...
    it("strips emoji and collapses punctuation into a stable slug", () => {
      expect(textProcessing.toSlug("Emoji 😊 Mix--Case & Stuff")).toBe(
        "emoji-mix-case-stuff",
      );
    });
  });
  ...
  describe("when wrapping text", () => {
    it("should wrap text to specified line length", () => {
      const longText =
        "This is a very long text that needs to be wrapped to multiple lines for better readability";
  ...
  describe("when generating log filenames", () => {
    ...
    it("should sanitize usernames in filenames", () => {
      const filename = textProcessing.generateLogFilename(
        "youtube",
        "user@#$%^&*()",
      );
    ...
    it("should handle different platforms", () => {
      const twitchFilename = textProcessing.generateLogFilename(
        "twitch",
        "user",
      );
      const youtubeFilename = textProcessing.generateLogFilename(
        "youtube",
        "user",
      );
  ...
  describe("when formatting log entries", () => {
    ...
    it("should handle different platforms", () => {
      ...
      const twitchEntry = textProcessing.formatLogEntry(
        timestamp,
        "user",
        "msg",
        "twitch",
      );
      const youtubeEntry = textProcessing.formatLogEntry(
        timestamp,
        "user",
        "msg",
        "youtube",
