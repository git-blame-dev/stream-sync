...
describe("YouTube Connection Manager - Lifecycle Behavior", () => {
  ...
  describe("Connection Addition", () => {
    ...
    it("should not replace existing connection when adding to same video ID", async () => {
      ...
      const result1 = await connectionManager.connectToStream(
        videoId,
        async () => connection1,
      );
      const result2 = await connectionManager.connectToStream(
        videoId,
        async () => connection2,
      );
  ...
  describe("Connection Removal & Recovery", () => {
    ...
    it("handles removal of non-existent connection gracefully", async () => {
      await expect(
        connectionManager.removeConnection("non-existent-video"),
  ...
  describe("Connection Status Management", () => {
    ...
    it("should handle setting ready status for non-existent connection", () => {
      ...
      expect(connectionManager.isConnectionReady("non-existent-video")).toBe(
        false,
      );
    });
  ...
  describe("Error Handling", () => {
    it("reports disconnect failures through the connection error channel", async () => {
      ...
      await expect(
        connectionManager.removeConnection("test-video"),
      ).resolves.toBeUndefined();
      ...
      expect(payloadArg).toEqual(expect.objectContaining({ videoId: "test-video" }));
      expect(messageArg).toBe("Error removing connection for video test-video");
