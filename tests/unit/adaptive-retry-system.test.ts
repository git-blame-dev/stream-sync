...
describe("Unified Adaptive Retry System", () => {
  ...
  describe("retrySystem.calculateAdaptiveRetryDelay()", () => {
    test("should calculate correct delays for exponential backoff", () => {
      ...
      expect(retrySystem.calculateAdaptiveRetryDelay("TikTok")).toBeCloseTo(
        2000,
      );
  ...
  describe("Real-world Connection Scenarios", () => {
    ...
    test("should handle multiple platforms failing simultaneously", () => {
      ...
      expect(retrySystem.calculateAdaptiveRetryDelay("Twitch")).toBeCloseTo(
        3380,
      );
      expect(retrySystem.calculateAdaptiveRetryDelay("YouTube")).toBeCloseTo(
        4394,
      );
    });

test("computes adaptive retry delays consistently across supported platforms", () => {
      const platforms = ["TikTok", "Twitch", "YouTube"];
...
test("exposes adaptive retry API and configuration contract", () => {
      ...
      expect(ADAPTIVE_RETRY_CONFIG.MAX_DELAY).toBeGreaterThan(
