...
const { YouTubeStreamDetectionService } = require('../../../src/services/youtube-stream-detection-service.ts');
...
describe('YouTubeStreamDetectionService behavior', () => {
    ...
    it('returns debug payload for unknown errors and marks retryable by default', async () => {
        const service = new YouTubeStreamDetectionService({}, { logger: noOpLogger });
        service._performDetection = createMockFn().mockRejectedValue(new Error('unexpected downstream issue'));
    ...
    it('updates timeout configuration and stays active while configured', () => {
        ...
        const updated = service.updateConfiguration({ timeout: 4500, username: 'test-user' });
