...
describe("CommandCooldownService", () => {
  ...
  describe("Per-User Command Cooldowns", () => {
    it("should allow command execution when no cooldown is active", () => {
      ...
      const canExecute = service.checkUserCooldown(
        userId,
        platformCooldown,
        heavyCooldown,
      );
    ...
    it("should track different cooldowns for different users", () => {
      ...
      const canUserBExecute = service.checkUserCooldown(
        userB,
        platformCooldown,
        heavyCooldown,
      );
      const canUserAExecute = service.checkUserCooldown(
        userA,
        platformCooldown,
        heavyCooldown,
      );
  ...
  describe("Heavy Command Detection", () => {
    ...
    it("should not count commands older than time window", () => {
      ...
      service.userCommandTimestamps.set(userId, [
        staleTimestamp,
        staleTimestamp - 100,
      ]);
  ...
  describe("Cooldown Configuration", () => {
    ...
    it("should keep fallback values for invalid config overrides", () => {
      ...
      expect(updated.heavyCommandThreshold).toBe(
        baseline.heavyCommandThreshold,
      );
  ...
  describe("Event-Driven Integration", () => {
    ...
    it("should update user cooldown timestamp when command executed", () => {
      ...
      expect(status.timeSinceLastCommand).toBeLessThanOrEqual(
        afterTime - beforeTime + 10,
      );
    });
