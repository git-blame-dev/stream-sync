...
describe("YouTube Live Stream Service - Channel ID User Experience", () => {
  ...
  describe("Username Input User Experience", () => {
    ...
    it("should handle resolution failures gracefully for users", async () => {
      ...
      mockInnertubeClient.resolveURL.mockRejectedValue(
        new Error("resolveURL timeout"),
      );
  ...
  describe("Invalid Channel ID Error Handling User Experience", () => {
    it("should handle malformed Channel ID gracefully for users", async () => {
      ...
      mockInnertubeClient.getChannel.mockRejectedValue(
        new Error("Invalid channel ID format"),
      );
...
it("resolves handles only for username input and skips it for Channel ID input", async () => {
    ...
    expect(usernameResult.streams[0]?.videoId).toBe("username123");
    ...
    expect(channelIdResult.streams[0]?.videoId).toBe("channelid123");
  });
  });
