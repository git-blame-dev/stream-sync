...
describe('youtube-channel-resolver', () => {
    ...
    it('returns null and reports error when channel handle is missing', async () => {
        const errors = [] as any[];
        const result = await resolveChannelId(null, '', { onError: (...args) => errors.push(args) });
    ...
    it('returns channel id input directly without resolver calls', async () => {
        const client = { resolveURL: () => Promise.resolve({ payload: { browseId: 'unused' } }) };
    ...
    it('returns null when resolver client is unavailable', async () => {
        const errors = [] as any[];
        const result = await resolveChannelId({}, '@creator', { onError: (...args) => errors.push(args) });
    ...
    it('resolves browse id and trims output', async () => {
        const client = {
            resolveURL: () => Promise.resolve({ payload: { browseId: ' UC' + 'c'.repeat(22) + ' ' } })
        };
    ...
    it('returns null when resolved payload has no browse id', async () => {
        ...
        const errors = [] as any[];
        const result = await resolveChannelId(client, '@MissingCreator', { onError: (...args) => errors.push(args) });
