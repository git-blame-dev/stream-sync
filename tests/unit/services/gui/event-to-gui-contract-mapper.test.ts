...
function createMapper(configOverrides = {}, optionsOverrides = {}) {
  ...
  } = require("../../../../src/services/gui/event-to-gui-contract-mapper");
...
describe("Event-to-GUI contract mapper behavior", () => {
  it("maps every supported source row to stable GUI kind contracts", async () => {
    ...
    const cases = [
      { type: "chat", expectedKind: "chat", data: { message: "hi" } },
      {
        ...
        data: { displayMessage: "user used command !test" },
      },
      {
        type: "greeting",
        expectedKind: "greeting",
        data: { displayMessage: "welcome" },
      },
      {
        type: "farewell",
        expectedKind: "farewell",
        data: { displayMessage: "bye" },
      },
      {
        type: "platform:follow",
        expectedKind: "notification",
        data: { displayMessage: "followed" },
      },
      {
        type: "platform:share",
        expectedKind: "notification",
        data: { displayMessage: "shared" },
      },
      {
        type: "platform:raid",
        expectedKind: "notification",
        data: { displayMessage: "raided" },
      },
      {
        type: "platform:gift",
        expectedKind: "notification",
        data: { displayMessage: "gifted" },
      },
      {
        type: "platform:paypiggy",
        expectedKind: "notification",
        data: { displayMessage: "subscribed" },
      },
      {
        type: "platform:giftpaypiggy",
        expectedKind: "notification",
        data: { displayMessage: "gifted memberships" },
      },
      {
        type: "platform:envelope",
        expectedKind: "notification",
        data: { displayMessage: "treasure chest" },
      },
    ];
        ...
        message: {
          ...
          parts: [
            { type: "text", text: "abc" },
            {
              ...
              imageUrl:
                "https://yt3.ggpht.example.invalid/test-600=w48-h48-c-k-nd",
            },
  ...
  it("resolves avatar by payload then cache then fallback", async () => {
    ...
    expect(payloadAvatar.avatarUrl).toBe(
      "https://example.invalid/payload-avatar.png",
    );
    expect(cachedAvatar.avatarUrl).toBe(
      "https://example.invalid/payload-avatar.png",
    );
  ...
  it("does not overwrite cached real avatar with fallback payload for the same platform user", async () => {
    ...
    expect(cachedAfterFallbackPayload.avatarUrl).toBe(
      "https://example.invalid/real-avatar.png",
    );
  });
  ...
  it("does not cache fallback payload avatars as canonical cache entries", async () => {
    ...
    expect(retained.avatarUrl).toBe(
      "https://example.invalid/retained-avatar.png",
    );
  });
