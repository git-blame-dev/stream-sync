  ...
  describe("Configuration with Getter Properties", () => {
    test("should correctly extract address from config object with getters", () => {
      const configWithGetters = {
        get address() {
          return "ws://custom-obs-server:4444";
        },
        get password() {
          return "secret123";
        },
...
test("connects to OBS with getter-derived address and password", async () => {
      const configWithGetters = {
        get address() {
          return "ws://production-obs:6666";
        },
        get password() {
          return "prod-secret";
        },
      ...
      const [connectedAddress, connectedPassword] = mockOBSInstance.connect.mock.calls[0] || [];
      expect(connectedAddress).toBe("ws://production-obs:6666");
      expect(connectedPassword).toBe("prod-secret");
    });
...
test("initializes OBS connection from getter-based config values", async () => {
      const configModuleStyle = {
        get address() {
          return "ws://init-test:9999";
        },
        get password() {
      ...
      const [connectedAddress, connectedPassword] = mockOBSInstance.connect.mock.calls[0] || [];
      expect(connectedAddress).toBe("ws://init-test:9999");
      expect(connectedPassword).toBe("init-password");
    });

    test("should not lose config values when using getConfig() method", () => {
      const getterConfig = {
        get address() {
          return "ws://config-test:3333";
        },
        get password() {
    ...
    test("should maintain connection state with getter-based enabled property", () => {
      const disabledConfig = {
        get address() {
          return "ws://disabled-server:2222";
        },
        get password() {
  ...
  describe("User Behavior Validation", () => {
    test("user should see OBS connection established with custom server from config", async () => {
      ...
      const [connectedAddress, connectedPassword] = mockOBSInstance.connect.mock.calls[0] || [];
      expect(connectedAddress).toBe("ws://streaming-pc:4455");
      expect(connectedPassword).toBe("stream-password");
