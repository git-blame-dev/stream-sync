...
describe("Centralized Endpoints Configuration", () => {
  describe("Twitch Endpoints", () => {
    ...
    test("should provide Twitch OAuth URLs", () => {
      ...
      expect(TWITCH.OAUTH.VALIDATE).toBe(
  ...
  describe("YouTube Endpoints", () => {
    ...
    test("should provide YouTube API methods", () => {
      ...
      const videosUrl = YOUTUBE.buildApiUrl("videos", {
        key: apiKey,
        id: "video123",
      });
      expect(videosUrl).toBe(
        "https://youtube.googleapis.com/youtube/v3/videos?key=test-api-key&id=video123",
      );

      const channelsUrl = YOUTUBE.buildApiUrl("channels", {
        key: apiKey,
        forUsername: "testuser",
      });
      expect(channelsUrl).toBe(
        "https://youtube.googleapis.com/youtube/v3/channels?key=test-api-key&forUsername=testuser",
      );
    });
  });
  ...
  describe("StreamElements Endpoints", () => {
    ...
    test("should provide StreamElements API base", () => {
      expect(STREAMELEMENTS.API_BASE).toBe(
        "https://api.streamelements.com/kappa/v2",
      );
    });
  ...
  describe("URL Building Utilities", () => {
    ...
    test("should build query parameters correctly", () => {
      ...
      expect(url).toBe(
        "https://youtube.googleapis.com/youtube/v3/search?key=api-key&channelId=UC123&type=video&order=date",
      );
    });
    ...
    test("should handle special characters in parameters", () => {
      ...
      expect(url).toBe(
        "https://youtube.googleapis.com/youtube/v3/search?key=api-key&q=search%20term%20with%20spaces",
      );
    });
  });
